...
fn translate_ja(ko: &'static str) -> &'static str {
    match ko {
        ...
        "항상 표시" => "常に表示",
        "게임 구동 중 씬 감지(Unknown) 결과와 상관없이 오버레이를 항상 표시합니다." => {
            "ゲーム実行中のシーン検出(Unknown)結果に関わらず、オーバーレイを常に表示します。"
        }
        "진단 및 디버그" => "診断とデバッグ",
        "디버그 창" => "デバッグウィンドウ",
        "디버그 모니터링 창 표시" => "デバッグモニタリングウィンドウを表示",
        "실시간 탐지 수치 및 진단 로그를 표출하는 디버그 창을 엽니다." => {
            "リアルタイム検出数値と診断ログを表示するデバッグウィンドウを開きます。"
        }
        "화면 캡처 설정" => "画面キャプチャ設定",
        "캡처 방식" => "キャプチャ方式",
        "기본값 / 안정적" => "デフォルト/安定",
        "실험적 / 고성능" => "実験的/高性能",
        "캡처 시 오버레이 보호" => "キャプチャ時のオーバーレイ保護",
        "화면 캡처 방지" => "画面キャプチャを防止",
        "해제 시 화면 캡쳐에 잡히는 대신, 특정 영역에 오버레이가 위치하면 곡 인식이 제대로 동작하지 않게 됩니다." => {
            "解除すると画面キャプチャに映るようになりますが、オーバーレイが特定領域にあると曲認識が正しく動作しなくなることがあります。"
        }
        ...
        // system/updater/windows.rs (MessageBoxW)
        "자동 패치가 완료되지 않았습니다." => "自動パッチが完了しませんでした。",
        "사유" => "理由",
        "새 앱 업데이트가 있습니다." => "新しいアップデートがあります。",
        "현재 버전" => "現在のバージョン",
        "최신 버전" => "最新バージョン",
        "지금 업데이트를 진행할까요?" => "今すぐアップデートしますか?",
