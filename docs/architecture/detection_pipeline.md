...
2. **Game Performance First**: 동적 폴링, 픽셀 체크섬 Early Return, 100% OCR-Free Pure Rust Native CV(템플릿 매칭 + 해시 + 히스토그램)로 인게임 프레임 드랍을 차단한다.
...
* **주요 모듈**: `rust/overmax_engine/src/detector/detection_pipeline.rs`, `rust/overmax_data/src/service/jacket_matcher.rs`, `rust/overmax_engine/src/detector/templates/logo.rs`
* **100% OCR-Free 씬 감지**: Windows OCR 의존성을 전면 소거. 재킷 영역 엣지 강도(`JACKET_EDGE_THRESHOLD = 15.0`) 또는 4px 우측 카테고리 띠 단색성(`check_category_band_solid`) 판정 및 Pure Rust 로고 템플릿 매칭으로 씬 판정 후 즉시 곡 매칭으로 진입.
...
* **주요 모듈**: `rust/overmax_engine/src/detector/play_state.rs`, `rust/overmax_engine/src/detector/templates/`
* 선곡창(Freestyle/OpenMatch)과 결과창(Result)을 독립 픽셀/템플릿 매칭으로 감지하여 캐시 전염 방지.
* **타입 안전 Enum 체계**: 모든 모드/난이도는 `Mode` (`B4`, `B5`, `B6`, `B8`) 및 `Difficulty` (`Normal`, `Hard`, `Maximum`, `SC`) enum을 활용하여 타입 안전성 확보.
* **버튼 모드**: 선곡창 ROI BGR 평균 색상 대조(<=60) / 결과창 독립 템플릿 매칭(`result_mode.rs`).
* **난이도**: 선곡창 패널 상대 밝기 / 결과창 독립 템플릿 매칭(`result_diff.rs`).
* **Max Combo 뱃지**: 사전 정의 대표 뱃지와 해시 거리 대조 (선곡창 <=10.0, 결과창 <=20.0).
* **Rate & Score**: 픽셀 체크섬(`compute_pixel_checksum`) 변화 시에만 200ms 간격 1-Pass 템플릿 매칭(`digit.rs`) 실행. Score 역산값(`Rate = Score / 10,000`) 대조를 통한 교차 검증 및 오차 자동 보정 (불일치 시 Score 우선 판정).
...
| `btn_mode` | 5x5 ROI BGR 평균 색상을 모드 대표 색상과 대조하여 버튼 모드(`Mode`) 감지 |
| `diff_panel` | 난이도 패널 영역(NM, HD, MX, SC)의 상대적 밝기 비교로 난이도(`Difficulty`) 감지 |
| `rate` | 선곡창 해당 패턴의 내 기록 판정률(Rate %) 템플릿 매칭 추출 |
| `score` | 선곡창 내 기록 점수(Score) 템플릿 매칭 추출 (Rate 검증용) |
...
| `mode_digit` / `openmatch_mode` | 결과창 전용 독립 템플릿 매칭으로 버튼 모드(`Mode`) 감지 (선곡창 캐시 의존 0%) |
| `diff_panel` / `openmatch_diff` | 결과창 전용 독립 템플릿 매칭으로 난이도(`Difficulty`) 감지 |
| `rate` | 이번 플레이의 최종 판정률(Rate %) 템플릿 매칭 추출 |
| `score` | 이번 플레이의 최종 점수(Score) 템플릿 매칭 추출 (Rate 교차 검증용) |
...
| ROI 이름 | ROI 목적 및 활용 방식 |
| :--- | :--- |
| `logo` | 상단 340x75 전역 ROI (로고 템플릿 매칭 및 ROI 좌표계 기준점 제공) |
...
캡처 백엔드가 제공하는 단 **한 장의 `CapturedFrame` 메모리 버퍼**를 통째로 파이프라인에 전달하며, 모든 ROI 크롭과 CV/템플릿 매칭 연산이 그 단일 프레임에서 동시에 실행된다. 프레임 찢어짐(Tearing)이나 서로 다른 프레임의 정보가 섞이지 않는다.
...
3. **Score ↔ Rate 교차 역산 검증**: Rate 템플릿 매칭과 Score 템플릿 매칭(`Rate = Score / 10,000`) 대조로 0.1% 미만 오차 자동 보정 및 100% 초과 이상치 제거 (불일치 시 Score 우선 적용).
...
* **Pure Rust CV 템플릿 매칭 Engine**: `rust/overmax_engine/src/detector/templates/`
